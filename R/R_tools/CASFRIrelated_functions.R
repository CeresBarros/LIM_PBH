...
## DETERMINE SPECIES TYPE (HARDWWOD/SOFTWOOD) 
...
## this funciton is only used in the UTM type SK forest inventories
...
## modif1: is the modifier of the first layer
## cover: is a vector of original crown closure 
##        of the three canopy layers (not CASFRI CC)

nonVegNatSK <- function(nonForest, modif1, cover,
                        MISSCODE = -1111L, ERRCODE = -9999L) {
  ## checks:
  if (length(unique(nonForest)) > 1 & 
      length(unique(modif1)) > 1) 
    stop("there should only be one nonForest/modifier unique value per stand")
  
  if (length(nonForest) > 3 | 
      length(cover) > 3 | 
      length(modif1) > 3)
    stop(" there should only be 3 different canopy layers")
  
  reps <- length(nonForest)
  nonForest <- unique(nonForest)
  modif1 <- unique(modif1)
  
  natNonVeg <- if (is.na(modif1)) nonForest else modif1
  
  if(!is.na(natNonVeg)) {
    if (grepl("L|R|FL", natNonVeg)) {
      corresp <- matrix(c("LA", "RI", "FL"), nrow = 3, ncol = 1,
                        dimnames = list(c("L", "R", "FL")))
      natNonVeg <- corresp[natNonVeg,]
      names(natNonVeg) <- NULL
    } else {
      if (sum(cover) < 10 & grepl("SL|WS|SL", natNonVeg)) {
        corresp <- matrix(c("SL", "WS", "SL"), nrow = 3, ncol = 1,
                          dimnames = list(c("CB", "SB", "SL")))
        natNonVeg <- corresp[natNonVeg,]
        names(natNonVeg) <- NULL
      } else 
        natNonVeg <- ERRCODE
  ...
  if (moist %in% "W") {
    if (is.na(nonForestLand)) {
      ...
      if (nonForestLand %in% c("S,O,N,S", "S,O,N,S", "M,O,N,G", "M,O,N,G", "F,O,N,G")) {
        ...
        if (!is.na(natNonVeg) & natNonVeg %in% "NMB")
...
## Alberta
ABToCASFRI <- function(inv, tablesDir, folder) {
  ...
  ## get inventory data
  casfriDT <- st_set_geometry(get(inv), NULL) %>%
    data.table(.)
  ...
  casfriDT[is.na(HEIGHT), `:=` (HEIGHT_UPPER = MISSCODE, 
                                HEIGHT_LOWER = MISSCODE)]
  casfriDT[HEIGHT_UPPER == MISSCODE & (is.na(SP1) | SP1 %in% c("SC", "SO")),
           `:=` (HEIGHT_UPPER = UNDEF, 
                 HEIGHT_LOWER = UNDEF)]
  casfriDT[HEIGHT_UPPER == MISSCODE & !(is.na(SP1) | SP1 %in% c("SC", "SO")) &
  ...
  temp <- invent2CASFRI(dt = casfriDT[NATURALLY_NON_VEG == ERRCODE], correspTab = NonFATable,
                        dtVar = "NAT_NON", correspVar = "AVI",
                        newVar = "CASFRI", newName = "NON_FORESTED_ANTHRO", keepOld = TRUE)
  casfriDT[NATURALLY_NON_VEG == as.character(ERRCODE) & is.na(NON_FORESTED_ANTHRO), NON_FORESTED_ANTHRO := temp[, NON_FORESTED_ANTHRO]]
  ...
  temp <- invent2CASFRI(dt = casfriDT[NON_FORESTED_ANTHRO == ERRCODE], correspTab = NatNVTable,
                        dtVar = "ANTH_NON", correspVar = "AVI",
                        newVar = "CASFRI", newName = "NATURALLY_NON_VEG", keepOld = TRUE)
  casfriDT[NON_FORESTED_ANTHRO == as.character(ERRCODE) & is.na(NATURALLY_NON_VEG), NATURALLY_NON_VEG := temp[, NATURALLY_NON_VEG]]
  ...
  casfriDT[, NFL := NFLAdjustAB(nonForestLand = NFL, sp1 = SP1, layerID = LAYER_RANK),
           by = "P_ID"]
  casfriDT[toupper(ANTH_NON) == "AIL" & !is.na(NFL), NFL := ANTH_NON]
  ...
  casfriDT[is.na(MOD1_YR) | MOD1_YR == 0, DIST1_YEAR :=  MISSCODE]
  casfriDT[is.na(MOD2_YR) | MOD2_YR == 0, DIST2_YEAR :=  MISSCODE]
  ...
  if ("MOD3_YR" %in% names(casfriDT)) {
    ...
    casfriDT[is.na(MOD3_YR) | MOD3_YR == 0, DIST3_YEAR :=  MISSCODE]
  ...
  casfriDT[is.na(SP1) &
             (((CROWN_CLOSURE_UPPER != MISSCODE | CROWN_CLOSURE_LOWER != MISSCODE) &
                 !is.na(DENSITY)) | HEIGHT_UPPER != MISSCODE | HEIGHT_LOWER != MISSCODE), 
           PRODUCTIVE_FOREST := "PP"]
  ## adjust species for PP
  spCols <- grep("SPEC[[:digit:]]*$", names(casfriDT), value = TRUE)
  casfriDT[PRODUCTIVE_FOREST == "PP",
  ...
  casfriDT[LAYER_RANK == 2 & DIST1 == "CO", PRODUCTIVE_FOREST := "PF"]
...
## Saskatchewan
SKToCASFRI <- function(inv, tablesDir, folder) {
  ...
  casfriDT <- st_set_geometry(saskatchewanfires_prefireMelt, NULL) %>%
    data.table(.)
  ...
  casfriDT[LAYER %in% c(1,2,3) & (HEIGHT_LOWER == ERRCODE | HEIGHT_UPPER == ERRCODE)] %>%
    .[is.na(SP1) & HEIGHT == 0, `:=` (HEIGHT_UPPER = MISSCODE, 
                                      HEIGHT_LOWER = MISSCODE)]
  ...
  # casfriDT[sum(SP1_COVER, SP2_COVER, SP3_COVER, SP4_COVER, SP5_COVER) == 90,
  #          SPEC6_PER := 10]
  ...
  casfriDT[NATURALLY_NON_VEG == as.character(ERRCODE) &
             NON_FORESTED_VEG == as.character(ERRCODE) &
             NON_FORESTED_ANTHRO == as.character(ERRCODE), 
  ...
  casfriDT[is.na(YOD_1) | YOD_1 == 0, DIST1_YEAR :=  MISSCODE]
  casfriDT[is.na(YOD_2) | YOD_2 == 0, DIST2_YEAR :=  MISSCODE]
  casfriDT[is.na(YOD_3) | YOD_3 == 0, DIST3_YEAR :=  MISSCODE]
  ...
  casfriDT[LAYER_RANK == 1 & is.na(SP1) &
             CROWN_CLOSURE_LOWER + HEIGHT_LOWER > 0 &
             any(DISTURBANCE_1 != "CO" & DISTURBANCE_2 != "CO" & DISTURBANCE_3 != "CO"), 
           PRODUCTIVE_FOREST := "PP"]
...
## Alberta
meltPreFireABInv <- function(inv, allVars, folder) {
  tmpDT <- st_set_geometry(get(inv), NULL) %>%
    data.table(.)
  ...
  eval(parse(text = paste0(
    "st_write(", invOut, ", dsn = file.path(folder, '", invOut, ".shp'), delete_layer = TRUE)"
  )))
...
## Saskatechewan
meltPreFireSKInv <- function(inv, folder) {
  tmpDT <- st_set_geometry(get(inv), NULL) %>%
    data.table(.)
