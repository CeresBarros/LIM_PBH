...
loadShpAndMakeValid <- function(file) {
  shapefile(file) %>% rgeos::gBuffer(byid = TRUE, width = 0)
}
...
shpStudyRegionCreate <- function(shpStudyRegionFull, studyArea, targetCRS) {
  ...
  if(class(studyArea) != "character" & (class(studyArea) == "SpatialPolygonsDataFrame" | class(studyArea) == "SpatialPolygons")) {
    shpStudyRegion <-  studyArea
    shpStudyRegion <- spTransform(shpStudyRegion, targetCRS)
  } else{
    if (studyArea != "FULL") {
      if (studyArea == "NWT") {
        ...
        shpStudyRegion <- spTransform(shpStudyRegionFullNWT, targetCRS)
