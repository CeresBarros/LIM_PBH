...
if (!exists("pkgDir")) {
  ...
  pkgDir <- file.path(
    if (Sys.info()[["sysname"]] == "Linux" && rver == "4.1") {
      "packages_docker"
    } else {
      "packages"
    },
...
Require::Require(
  c("data.table", "dplyr", "SpaDES.core", "ToolsCB"),
  install = FALSE
)
...
if (
  grepl("for-cast", Sys.info()["nodename"]) ||
...
if (mergeDMCPSME) {
  ...
  allPixelCohortDataMnt[
    vegTypeCN %in% c("DMCPSME", "PSME", "dryPSME"),
    vegTypeCN := "DMCPSME"
  ]
}
...
fireHVdata <- cbind(
  fireHVdata,
  summaryFireAttributes[, .(scenario, rep, pixelIndex, vegTypeCN)]
)
...
lapply(
  split(fireHVdata, by = c("rep", "vegTypeCN"), drop = TRUE),
  FUN = function(allData, HVoutputPath, doAll) {
    ...
    if (!doAll) {
      ...
      computedHVpairs <- grep(
        paste0(pattern = file.suffix, "_Intersection.*(1|2|3).rds$"),
        list.files(HVoutputPath),
        value = TRUE
      )
  ...
  doAll = doAll
) ## if doAll == FALSE, only missing HV intersection pairs will be computed
...
doAll <- FALSE ## if doAll == FALSE, only missing HV intersection pairs will be computed
lapply(
  split(fireHVdata, by = c("rep"), drop = TRUE),
  FUN = function(allData, HVoutputPath, doAll) {
    r <- unique(allData$rep)
...
vegDataForHVs[,
  dummyHVid := ifelse(
    useFirstLastYear,
    paste(scenario, rep, year, vegTypeCN, sep = "_"),
...
vegHVdata <- cbind(
  vegHVdata,
  vegDataForHVs[, .(scenario, rep, year, pixelIndex, vegTypeCN)]
)
...
if (mergeDMCPSME & doMergedOnly) {
  ## only do the the merged vegTypes
...
## subset first 4 axes:
cols <- c(
  grep("PC(1|2|3|4)", names(vegHVdata), value = TRUE),
...
if (useFirstLastYear) {
  pixelIndexList <- split(
    unique(vegHVdata[year == max(yearSubset), .(rep, vegTypeCN, pixelIndex)]),
    by = c("rep", "vegTypeCN"),
    drop = TRUE
  )
  ...
  if (any(!test)) {
    stop("different veg types at start year fround across scenarios.")
  }
} else {
  ...
  pixelIndexList <- unique(pixelIndexList[, c(
    "rep",
    "vegTypeCN",
    "pixelIndex"
  )])
...
if (useFirstLastYear) {
  ...
  future::plan(future::multisession, gc = TRUE, workers = 5)
  future.apply::future_lapply(
    split(vegHVdata, by = c("rep", "scenario")),
    FUN = function(allData, HVoutputPath, doAll) {
      r <- unique(allData$rep)
      scen <- unique(allData$scenario)
