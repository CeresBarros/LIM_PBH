...
doEvent.LBMR = function(sim, eventTime, eventType, debug = FALSE) {
  ...
  switch(eventType, 
         ...
         fireDisturbance = {
           if(!is.null(sim$rstCurrentBurn)) {   ## Ceres, include this condition to the event scheduler outside the event
         ...
         cohortAgeReclassification = {
           # if(P(sim)$successionTimestep != 1){   ## Ceres: this shouldn't be necessary, because the event is only scehduled under this condition 
