...
#    `functionName` is a function's name in your module (e.g., `fireSeverityEvent1`).
# 2. Copy this file to the tests folder (i.e., `D:/Cherry/GitHub/LandscapesInMotion/R/SpaDES/m/fireSeverity/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("fireSeverity")
  ...
  parameters <- list(
    ...
    fireSeverity = list(.saveInitialTime = NA)
  )
  ...
  if (exists("fireSeverityEvent1", envir = .GlobalEnv)) {
    simOutput <- fireSeverityEvent1(mySim)
  } else {
  ...
  if (exists("fireSeverityEvent2", envir = .GlobalEnv)) {
    simOutput <- fireSeverityEvent2(mySim)
  } else {
