...
#    `functionName` is a function's name in your module (e.g., `simplifyLCCVegEvent1`).
# 2. Copy this file to the tests folder (i.e., `D:/Cherry/GitHub/LandscapesInMotion/R/SpaDES/m/simplifyLCCVeg/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("simplifyLCCVeg")
  path <- list(modulePath = "D:/Cherry/GitHub/LandscapesInMotion/R/SpaDES/m",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    simplifyLCCVeg = list(.saveInitialTime = NA)
  )
  ...
  if (exists("simplifyLCCVegEvent1", envir = .GlobalEnv)) {
    simOutput <- simplifyLCCVegEvent1(mySim)
  } else {
  ...
  if (exists("simplifyLCCVegEvent2", envir = .GlobalEnv)) {
    simOutput <- simplifyLCCVegEvent2(mySim)
  } else {
