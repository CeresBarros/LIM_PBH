...
#    `functionName` is a function's name in your module (e.g., `simpleLCCSuccessionEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Ceres/GitHub/LandscapesInMotion/R/SpaDES/m/simpleLCCSuccession/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("simpleLCCSuccession")
  ...
  parameters <- list(
    ...
    simpleLCCSuccession = list(.saveInitialTime = NA)
  )
  ...
  if (exists("simpleLCCSuccessionEvent1", envir = .GlobalEnv)) {
    simOutput <- simpleLCCSuccessionEvent1(mySim)
  } else {
  ...
  if (exists("simpleLCCSuccessionEvent2", envir = .GlobalEnv)) {
    simOutput <- simpleLCCSuccessionEvent2(mySim)
  } else {
