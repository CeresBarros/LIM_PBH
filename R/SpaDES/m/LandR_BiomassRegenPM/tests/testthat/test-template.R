...
#    `functionName` is a function's name in your module (e.g., `LandR_BiomassRegenPMEvent1`).
# 2. Copy this file to the tests folder (i.e., `E:/GitHub/LandscapesInMotion/R/SpaDES/m/LandR_BiomassRegenPM/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("LandR_BiomassRegenPM")
  ...
  parameters <- list(
    ...
    LandR_BiomassRegenPM = list(.saveInitialTime = NA)
  )
  ...
  if (exists("LandR_BiomassRegenPMEvent1", envir = .GlobalEnv)) {
    simOutput <- LandR_BiomassRegenPMEvent1(mySim)
  } else {
  ...
  if (exists("LandR_BiomassRegenPMEvent2", envir = .GlobalEnv)) {
    simOutput <- LandR_BiomassRegenPMEvent2(mySim)
  } else {
