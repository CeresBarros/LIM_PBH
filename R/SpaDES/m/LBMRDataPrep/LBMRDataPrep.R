...
defineModule(sim, list(
  ...
  keywords = c("LANDIS biomass model R", "vegetation succession", "spatial explicit"),
  ...
  documentation = list("README.txt", "LBMRDataPrep.Rmd"),
  ...
  inputObjects = bind_rows(
    ...
    expectsInput(objectName = "landCover", objectClass = "RasterLayer",
                 desc = "Land cover map in study area, default is Canada National LCC2010",
...
doEvent.LBMRDataPrep = function(sim, eventTime, eventType, debug = FALSE) {
  if (eventType == "init") {
    ...
    sim <- scheduleEvent(sim, P(sim)$.plotInitialTime, "LBMRDataPrep", "plot")
    sim <- scheduleEvent(sim, P(sim)$.saveInitialTime, "LBMRDataPrep", "save")
...
Init <- function(sim) {
  ...
  #simulationMaps <- sim$nonActiveEcoregionProducerCached(nonactiveRaster = sim$landCover,
  simulationMaps <- Cache(nonActiveEcoregionProducer, nonactiveRaster = sim$landCover,
  ...
  sim$minRelativeB <- data.frame(ecoregion = sim$ecoregion[active == "yes",]$ecoregion,
                                 ...
                                 X4 = 0.7, X5 = 0.9)
  message("Done LBMRDataPrep: ", Sys.time())
...
.inputObjects = function(sim) {
  ...
  ## TO_CHANGE - should be using checksums
  
  dataPath <- file.path(modulePath(sim), "LBMRDataPrep", "data")
  fileNames1 <- c("ecodistricts.dbf", "ecodistricts.prj", "ecodistricts.sbn", "ecodistricts.sbx",
                  ...
                  "landCover_V1_4a.tif", "NFI_MODIS250m_kNN_Species_Abie_Bal_v0.tif",
  ...
  # landCover may be loaded by other modules
  
  landCoverFilename <- if(!is.null(sim$landCover)) filename(sim$landCover) else file.path(dataPath, "NA_LandCover_2010_25haMMU.tif")
  
  
  if(needDownload) {
    suppressWarnings(checkTable <- data.table(downloadData(module = "LBMRDataPrep",
                                                           path = modulePath(sim))))
    ...
    # landCover
    if(!("NA_LandCover_2010_25haMMU.tif" %in% checkContent_passed)){
      
      file.name <- grep("NA_LandCover_2010_25haMMU.tif$",
                        unzip(file.path(dataPath, "Land_Cover_2010_TIFF.zip"), list = TRUE)$Name,
                        value = TRUE)
      
      unzip(zipfile = file.path(dataPath, "Land_Cover_2010_TIFF.zip"),
            files = file.name,
            exdir = dataPath, junkpaths = TRUE)
    } 
  }
  ...
  if(is.null(sim$landCover)) sim$landCover <- raster(landCoverFilename)
  ...
  # projection(sim$landCover) <- projection(sim$specieslayers)
  ...
  if(needShrinking) {
    if(!is.null(sim$shpStudyRegionFull)) {
      ...
      # rasters
      # landCover
      sim$landCover <- crop(sim$landCover, sim$shpStudyRegionFull,
                            filename = file.path(dirname(landCoverFilename), paste0("Small",basename(landCoverFilename))),
                            overwrite=TRUE)
      #file.remove(dir(dirname(landCoverFilename), full.names = TRUE) %>%
      #              .[grep(basename(.), pattern = paste0("^",basename(landCoverFilename)))])
      file.rename(filename(sim$landCover), landCoverFilename)
      sim$landCover@file@name <- landCoverFilename
