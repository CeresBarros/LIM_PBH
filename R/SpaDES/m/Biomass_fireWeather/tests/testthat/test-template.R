...
#    `functionName` is a function's name in your module (e.g., `Biomass_fireWeatherEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Ceres/GitHub/LandscapesInMotion/R/SpaDES/m/Biomass_fireWeather/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("Biomass_fireWeather")
  ...
  parameters <- list(
    ...
    Biomass_fireWeather = list(.saveInitialTime = NA)
  )
  ...
  if (exists("Biomass_fireWeatherEvent1", envir = .GlobalEnv)) {
    simOutput <- Biomass_fireWeatherEvent1(mySim)
  } else {
  ...
  if (exists("Biomass_fireWeatherEvent2", envir = .GlobalEnv)) {
    simOutput <- Biomass_fireWeatherEvent2(mySim)
  } else {
