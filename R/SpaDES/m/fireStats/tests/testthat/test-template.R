...
#    `functionName` is a function's name in your module (e.g., `fireStatsEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Ceres/GitHub/LandscapesInMotion/R/SpaDES/m/fireStats/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("fireStats")
  ...
  parameters <- list(
    ...
    fireStats = list(.saveInitialTime = NA)
  )
  ...
  if (exists("fireStatsEvent1", envir = .GlobalEnv)) {
    simOutput <- fireStatsEvent1(mySim)
  } else {
  ...
  if (exists("fireStatsEvent2", envir = .GlobalEnv)) {
    simOutput <- fireStatsEvent2(mySim)
  } else {
