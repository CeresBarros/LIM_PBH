...
if (
  paste0(version$major, ".", strsplit(version$minor, "[.]")[[1]][1]) != "4.1"
) {
...
traitsTable <- data.frame(
  traitsTable[, .(longevity, shadetolerance, firetolerance, postfireregen)],
...
if (useFirstLastYear) {
  vegData <- allPixelCohortDataMnt[
    year %in% c(min(yearSubset), max(yearSubset))
  ]
} else {
...
vegData <- vegData[,
  list(B = sum(B)),
  by = c("speciesCode", "scenario", "rep", "year", "pixelIndex")
]
vegData <- dcast.data.table(
  vegData,
  as.formula("... ~ speciesCode"),
  value.var = "B"
)
...
traitCWMs <- Cache(
  ...
  rep = combos$rep,
  yr = combos$year,
  MoreArgs = list(vegData = vegData, traitsTable = traitsTable, spp = spp),
...
traitCWMs <- traitCWMs[
  vegData[, .(scenario, rep, year, pixelIndex)],
...
vegHVPCA <- grep(
  "vegHVs",
  list.files(HVoutputPath, "OrdinationObj", full.names = TRUE),
  value = TRUE
)
...
for (i in 1:nrow(vegHVPCA$rotation)) {
  loadings_coords <- rbind(
    loadings_coords,
    rbind(c(0, 0, 0), vegHVPCA$rotation[i, 1:3])
  )
}
loadings_coords <- as.data.table(cbind(
  as.data.frame(loadings_coords),
  rep(rownames(vegHVPCA$rotation), each = 2)
))
...
Vars <- unique(unlist(loadings_coords[,
  lapply(.SD, function(x) which(abs(x) >= 0.5)),
...
for (i in 1:nrow(fortify(trait.fit))) {
  traits_coords <- rbind(
    ...
    rbind(
      c(0, 0, 0),
      as.data.frame(fortify(trait.fit))[i, c("PC1", "PC2", "PC3")]
    )
  )
}
...
trts <- unique(unlist(traits_coords[,
  lapply(.SD, function(x) which(abs(x) >= 0)),
  .SDcols = c("PC1", "PC2", "PC3")
]))
...
pixelIndexDT <- vegHVPCAscores[
  year == max(yearSubset),
  .(rep, scenario, vegTypeCN, pixelIndex)
]
...
lapply(
  ...
  loadings_coords = as.data.frame(loadings_coords[
    Var %in% Vars,
    .(PC1, PC2, PC3)
  ]) *
    ordiArrowMul(vegHVPCA, fill = 2, choices = 1:3, display = "species"),
  PHvect_coords = as.data.frame(
    traits_coords[Label %in% trts, .(PC1, PC2, PC3)] *
      ordiArrowMul(trait.fit, choices = 1:3)
  ), ## ordiArrowMul finds the appropriate multiplifer to plot axes.
...
vegHVPCA <- grep(
  "vegHVs",
  list.files(HVoutputPath2, "OrdinationObj", full.names = TRUE),
  value = TRUE
)
vegHVPCA <- readRDS(vegHVPCA)
vegHVPCAscores <- cbind(as.data.table(vegHVPCA$x), vegDataForHVs)
...
ggplot(
  plotData[grepl("PSME", vegTypeCN)],
  aes(log(PC1), PC2, colour = vegTypeCN, shape = as.factor(year))
) +
...
ggplot(
  plotData[grepl("PSME", vegTypeCN) & scenario == "PM"],
  aes(PC1, PC3, colour = as.factor(rep), shape = scenario)
) +
  geom_point(alpha = 0.5) +
  if (useFirstLastYear) {
...
ggplot(
  plotData[grepl("PSME", vegTypeCN) & scenario == "PM"],
  aes(PC1, PC2, colour = as.factor(rep), shape = scenario)
) +
  geom_point(alpha = 0.5) +
  if (useFirstLastYear) {
    facet_wrap(year ~ vegTypeCN)
...
PSMEPCAplot <- autoplot(
  plotDataPCA,
  data = plotData[grepl("PSME", vegTypeCN)],
  colour = col,
  shape = shape,
  label.colour = "blue",
  loadings = TRUE,
